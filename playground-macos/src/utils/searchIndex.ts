// Search index for Spotlight: only About, News, and ArtworkGallery
// Each entry: { title, description, section, appId }

const searchIndex = [
  // About
  {
    title: "About Rahaf Abutarbush",
    description: "Emerging Technology, Instinct-led Innovation, Market Relevance.",
    section: "About",
    appId: "about"
  },
  {
    title: "Expertise: Design Thinking",
    description:
      "I lead ideation sessions that push technology beyond the buzzwords. It's not just about using the latest tools, it's about applying them with intent.",
    section: "Expertise",
    appId: "about"
  },
  {
    title: "Expertise: Product Management",
    description:
      "I thrive in leading product strategy and management in fast-moving spaces like emerging tech, where success isn't about racing to keep up, it's about building with purpose.",
    section: "Expertise",
    appId: "about"
  },
  {
    title: "Expertise: Public Speaking",
    description:
      "I've always been drawn to what makes someone pay attention, or remember something after the meeting is over. Storytelling is about clarity and answering the core question: 'What's in it for us?'",
    section: "Expertise",
    appId: "about"
  },
  // News (from News.tsx)
  {
    title:
      "PwC Middle East in Qatar, Microsoft host conversation on gender balance in technology industry",
    description: "The Peninsula Qatar, 2024",
    section: "News",
    appId: "news"
  },
  {
    title: "New virtual economy: What does the rise of the metaverse mean?",
    description: "Euronews, 2022",
    section: "News",
    appId: "news"
  },
  {
    title: "Students Present Demos for Tech Startup Launchpad Course",
    description: "CMU-Q News, 2024",
    section: "News",
    appId: "news"
  },
  {
    title: "Carnegie Mellon Qatar Celebrates Student Success",
    description: "Gulf Times, 2022",
    section: "News",
    appId: "news"
  },
  {
    title: "Carnegie Mellon Celebrates Women in STEM Fields",
    description: "CMU-Q News, 2024",
    section: "News",
    appId: "news"
  },
  {
    title: "Carnegie Mellon Students Win National Debate Contest",
    description: "The Peninsula Qatar, 2022",
    section: "News",
    appId: "news"
  },
  {
    title: "CMU-Q team wins Arabic University debate championship",
    description: "Qatar Tribune, 2022",
    section: "News",
    appId: "news"
  },
  {
    title: "CMU-Q student club overcomes obstacles to host TEDx event",
    description: "CMU-Q News, 2021",
    section: "News",
    appId: "news"
  },
  {
    title:
      "On UN Arabic Language Day, Top CMU-Q Team Reflects on Growth of Arabic Debate",
    description: "CMU-Q News, 2022",
    section: "News",
    appId: "news"
  },
  // ArtworkGallery (Events Gallery)
  {
    title: "LEAP 2024 - KSA",
    description: "Future of XR - LEAP 2024 - KSA - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "Women in STEM Panel",
    description:
      "Panel for women pursuing careers in STEM - Carnegie Mellon 2023 - Qatar - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "Women in Tech",
    description: "PwC X Microsoft Women in Tech - PwC 2024 - Qatar - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "Ru'ya",
    description:
      "Showcasing prototypes and innovations - Ru'ya 2024 - UAE - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "IDFE DEALS",
    description:
      "Demonstrating Soft Skill VR training - IDFE 2023 - Spain - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "Emerging Tech Lab",
    description:
      "Lab Tour & Presentations - PwC Emerging Tech Lab 2022 - Qatar/UAE - Events Gallery",
    section: "Events Gallery",
    appId: "artwork-gallery"
  },
  {
    title: "Resume",
    description: "Rahaf Abutarbush Resume - Resume",
    section: "Resume",
    appId: "preview",
    url: "img/ui/Rahaf_Abutarbush.pdf"
  },
  {
    title: "CV",
    description: "Rahaf Abutarbush CV",
    section: "Resume",
    appId: "preview",
    url: "img/ui/Rahaf_Abutarbush.pdf"
  }
];

export default searchIndex;
